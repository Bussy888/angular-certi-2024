...
export class CityService {
  ...
  addCity(name: string) {
    // Convertimos el nombre de la ciudad a TitleCase
...
function toTitleCase(str: string): string {
  return str
    ...
    .split(' ') // Dividimos el string en palabras
    .map(word => word.charAt(0).toUpperCase() + word.slice(1)) // Convertimos la primera letra de cada palabra a mayúscula
    .join(' '); // Volvemos a juntar las palabras en un solo string
}